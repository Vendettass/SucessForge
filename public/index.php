...
if (!empty($_GET['appid'])) {
    ...
    if ($appid > 0) {
        ...
        if (!$appDetails) {
            ...
            // Si un joueur est indiqué, on bascule en mode "profil"
            if (!empty($steamInput)) {
...
<body class="bg-slate-900 text-slate-100 min-h-screen">
    <header class="border-b border-slate-800 bg-slate-950/80 backdrop-blur sticky top-0">
        <div class="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
            ...
            <p class="text-sm text-slate-400">Explore les succ�s Steam</p>
        </div>
    </header>
    ...
    <main class="max-w-5xl mx-auto px-4 py-8 space-y-6">
            ...
            <form method="GET" class="flex flex-col gap-3">
                ...
                <div class="flex flex-col sm:flex-row gap-3">
                    <input
                        type="text"
                        name="steam"
                        placeholder="SteamID64 ou URL personnalisée (optionnel)"
                        class="flex-1 px-3 py-2 rounded-lg bg-slate-800 border border-slate-700 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        ...
        <?php if ($appDetails && !empty($achievements)): ?>
            <section class="mt-6">
                <h3 class="text-xl font-semibold mb-3">Succ�s du jeu</h3>
                <p class="text-sm text-slate-400 mb-4">
                    Tri�s par raret� (les plus rares en premier).
                </p>
        ...
        <?php elseif ($appid && !$error): ?>
            <section class="mt-6">
                <p class="text-slate-400 text-sm">
                    Aucun succ�s � afficher pour ce jeu.
                </p>
            </section>
        <?php endif; ?>
    </main>

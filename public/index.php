    ...
    <main class="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <section>
            ...
            <form method="GET" class="flex flex-col gap-3">
                ...
                <?php if ($user): ?>
                    <form method="post" action="/save_steam.php" class="mt-2 flex gap-2">
                        <input
                            name="steamid"
                            placeholder="SteamID64"
                            class="px-3 py-2 rounded-lg bg-slate-800 border border-slate-700"
                            value="<?php echo htmlspecialchars($steamInput); ?>">
                        <button class="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-500 font-semibold">
                            Enregistrer mon SteamID
                        </button>
                    </form>
                <?php else: ?>
                    <p class="text-sm text-slate-400 mt-2">
                        <a href="/login.php" class="text-indigo-400 underline">Se connecter</a>
                        pour enregistrer votre SteamID.
                    </p>
                <?php endif; ?>
            </form>
        </section>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="w-full h-3 rounded-full bg-slate-800 overflow-hidden">
                            <div
                                ...
                                style="width: <?php echo $totalAchievements > 0 ? ($unlockedCount / $totalAchievements) * 100 : 0; ?>%;"></div>
                        </div>
                    </div>
                <?php endif; ?>
